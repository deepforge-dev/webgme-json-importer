...
import type { GMERelationRuleType } from './NodeState';
...
export class Exporter {
    ...
    async _metaDictToGuids(
        meta_dict: Core.RelationRule | null
    ): Promise<GMERelationRuleType> {
        if (meta_dict === null) return meta_dict;
        return Object.fromEntries(
            await Promise.all(
                Object.entries(meta_dict).map(async ([key, value]) => {
    ...
    async toJSON(
        node: Core.Node,
        omit: OmittedProperties | boolean = new OmittedProperties()
    ): Promise<Partial<NodeState>> {
    ...
    async _toJSON(
        ...
        Object.keys(json).forEach((key) => {
            if (this[key]) {
                // ToDo: Fix this not to use dynamic typings
    ...
    attributes(
        node: Core.Node,
        json: Pick<NodeState, 'attributes'>,
        _promiseQueue: Promise<void>[],
        _omitted: Set<string>
    ) {
    ...
    attribute_meta(
        node: Core.Node,
        json: Pick<NodeState, 'attribute_meta'>,
        _promiseQueue: Promise<void>[],
        _omitted: Set<string>
    ) {
    ...
    sets(
        node: Core.Node,
        json: Pick<NodeState, 'sets' | 'member_attributes' | 'member_registry'>,
            ...
            ...this.core
                ...
                .map(async (name) => {
                    ...
                    const members = await Promise.all(
                        paths.map((path) =>
                            this.core.loadByPath(this.rootNode, path)
                        )
                    );
                    const memberGuids = members.map((member) =>
                        this.core.getGuid(member)
                    );
                    json.sets[name] = memberGuids;

                    if (!omitted.has('member_attributes')) {
                        // Alternatives to this closure variable?
                        members.forEach((member) => {
                            ...
                            this.core
                                .getMemberAttributeNames(node, name, memberPath)
                                .forEach((attrName) => {
                                    const value = this.core.getMemberAttribute(
                                        node,
                                        name,
                                        memberPath,
                                        attrName
                    ...
                    if (!omitted.has('member_registry')) {
                        members.forEach((member) => {
                            ...
                            this.core
                                .getMemberRegistryNames(node, name, memberPath)
                                .forEach((regName) => {
                                    ...
                                    json.member_registry[name][guid][regName] =
                                        value;
                                });
                        });
                    }
    ...
    pointers(
        node: Core.Node,
        json: Pick<NodeState, 'pointers'>,
        promiseQueue: Promise<void>[],
        _omitted: Set<string>
    ) {
        promiseQueue.push(
            ...this.core.getOwnPointerNames(node).map(async (name) => {
                const path = this.core.getPointerPath(node, name);
                if (path) {
                    const target = await this.core.loadByPath(
                        this.rootNode,
                        path
                    );
    ...
    children_meta(
        node: Core.Node,
        json: Pick<NodeState, 'children_meta'>,
        promiseQueue: Promise<void>[],
        _omitted: Set<string>
    ) {
        promiseQueue.push(
            this._metaDictToGuids(this.core.getChildrenMeta(node)).then(
                (children_meta) => {
    ...
    pointer_meta(
        node: Core.Node,
        json: Pick<NodeState, 'pointer_meta'>,
        promiseQueue: Promise<void>[],
        _omitted: Set<string>
    ) {
        promiseQueue.push(
            ...this.core.getOwnValidPointerNames(node).map(async (name) => {
    ...
    mixins(
        node: Core.Node,
        json: Pick<NodeState, 'mixins'>,
        _promiseQueue: Promise<void>[],
        _omitted: Set<string>
    ) {
        json.mixins = Object.values(this.core.getMixinNodes(node)).map((node) =>
